...
export const TableSortArrows = ({ onClick, id, manager, onTopClick = null, onBottomClick = null}) => {

  const [filter, setFilter] = useState(null)
  const onTopArrowClick = () => {
    setFilter("TOP")
    manager.resetArrows(id)
    if(onTopClick)
    {
      onTopClick()
    }
  ...
  return (
    <svg className="table-vertical-arrows"width="11" height="15" viewBox="0 0 11 15" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path onClick={onTopArrowClick}
        ...
        fill={filter === "TOP" ? '#A5A5A5' : '#D3D6E4'} style={{cursor: "pointer"}}
      />
      <path onClick={onBottomArrowClick}
        ...
        fill={filter === "BOTTOM" ? '#A5A5A5' : '#D3D6E4'} style={{cursor: "pointer"}}
      />
    </svg>
  );
};
